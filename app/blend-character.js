...
THREE.BlendCharacter = function() {
	...
	this.loadJSON = function(url, onLoad) {
		...
		loader.load(url, function(geometry, materials) {
			let originalMaterial = materials[0];
			originalMaterial.skinning = true;

			THREE.SkinnedMesh.call(scope, geometry, originalMaterial);

			let mixer = new THREE.AnimationMixer(scope);
			scope.mixer = mixer;
	...
	this.crossfade = function(fromAnimName, toAnimName, duration) {
		...
		let fromAction = this.play(fromAnimName, 1);
		let toAction = this.play(toAnimName, 1);
	...
	this.applyWeight = function(animName, weight) {
		this.mixer.clipAction(animName)
			.setEffectiveWeight(weight);
	};
